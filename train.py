...
def trainIters(lang, modelData, encoder, decoder, args):
    """
    Main training loop that runs for a specified number of iterations.
    Args:
        lang: Language instance for vocabulary management
        modelData: ModelData instance containing training data
        encoder: Encoder neural network model
        decoder: Decoder neural network model
        args: Training configuration arguments
    Returns:
        tuple: Lists of training losses and validation scores
    """
    start = time.time()
    plot_losses = []
...
def train(input_tensor, target_tensor, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion, args):
    """
    Performs one iteration of training on a single input/target pair.
    Args:
        input_tensor: Input sequence tensor
        target_tensor: Target sequence tensor
        encoder: Encoder neural network model
        decoder: Decoder neural network model
        encoder_optimizer: Optimizer for encoder parameters
        decoder_optimizer: Optimizer for decoder parameters
        criterion: Loss function
        args: Training configuration arguments
    Returns:
        float: Average loss for this training iteration
    """
