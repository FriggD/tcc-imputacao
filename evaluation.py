...
def evaluateRandomly(pairs, lang, encoder, decoder, args, n=0):
    """
    Evaluates the model on randomly selected pairs from the dataset.
    Args:
        pairs: List of input/target pairs
        lang: Language instance for vocabulary
        encoder: Trained encoder model
        decoder: Trained decoder model
        args: Evaluation configuration
        n (int): Number of random pairs to evaluate (default=0)
    """
    for i in range(n):
...
def evaluateAll(modelData, lang, encoder, decoder, args, n=None, computeMafDist=False):
    """
    Evaluates the model on all pairs in the dataset or a subset.
    Args:
        modelData: ModelData instance containing evaluation data
        lang: Language instance for vocabulary
        encoder: Trained encoder model
        decoder: Trained decoder model
        args: Evaluation configuration
        n (int, optional): Number of pairs to evaluate
        computeMafDist (bool): Whether to compute MAF distribution
    Returns:
        tuple: Evaluation metrics including predictions and scores
    """
    acertos = 0
    tentativas = 0
    if n:
...
def evaluate(lang, encoder, decoder, sentence, args):
    ...
    Args:
        ...
        args: Evaluation configuration
    Returns:
        tuple: Decoded output and attention weights
    """
    with torch.no_grad():
